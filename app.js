...
function handleSpeedPeaks(answer) {
    ...
    if (appState.hasSpeedPeaks) {
        ...
        calculateBonus(); // Calcula bônus sem picos
        renderFinalScreen();
    }
}
...
function calculateContractDays(startDateInput, endDateInput) {
    ...
    appState.totalDays = Math.ceil(totalHours / 24); // Arredonda para cima
}

// Calcula o bônus base (sem considerar picos)
function calculateBonus() {
    // Bônus = 100 por semana completa (7 dias), arredondado para cima.
    // Mínimo de 100.
    appState.bonus = Math.ceil(appState.totalDays / 7) * 100;
    if (appState.bonus < 100) {
        appState.bonus = 100; // Garante o mínimo
    }
}

// Calcula o bônus final considerando os picos
function calculateBonusWithPeaks(peakDates) {
    calculateBonus(); // Calcula o bônus base primeiro
    ...
    // Deduz 100 para cada semana que teve pelo menos um pico
    const deduction = weeksWithPeaks.size * 100;
    appState.bonus -= deduction;
